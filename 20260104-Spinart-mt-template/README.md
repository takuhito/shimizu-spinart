...
### 実装のポイント
1.  **動的な年度取得**: `<mt:ArchiveList type="Yearly">` を使用し、記事が投稿されている年度を自動的にループ処理します。
2.  **既存デザインの保持**: 元の Bootstrap ベースのドロップダウン構造およびボタン構造をそのまま維持しながら、内部のリンク生成のみを自動化しました。
3.  **再構築エラーの防止**: `<$mt:ArchiveLink$>` は MT のアーカイブ設定に依存するため、環境に左右されないよう `/contents/<$mt:ArchiveTitle$>/` という形式でパスを直接組み立てる方式を採用しました。
